...
Ambient и Downtempo. Текущая версия `0.21.0` — проверяемый DSP-прототип с
...
- Новый Character `Current` рассматривает восемь FDN-линий как точки одного
  жидкого объёма. Общее rank-2 поле согласованно двигает fractional delay,
  damping и полную L/R-позицию каждой линии без набора независимых LFO.
- Decay снова управляет только feedback RT60: экспериментальная
  `sqrt(1 − g²)`-компенсация удалена, поэтому базовая FDN injection снова
  остаётся постоянной при увеличении времени. Длинный хвост набирает плотность
  только за счёт накопления энергии внутри сети.
- Lateral Decay без новой ручки связывает Width и Evolution с мягким
  удлинением боковой группы линий: до `+14%` при Width `200%`, сохраняя
  центральное mono-ядро и плавно исчезая во Freeze.
