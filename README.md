...
Ambient и Downtempo. Текущая версия `0.7.0` — проверяемый DSP-прототип на C++20,
JUCE 8.0.14 и CMake.
...
- единый Drift, плавно развивающийся от воздушного движения к выраженным
  body/presence trajectories максимального Evolution;
...
stereo input
    ...
    -> Drift: Evolution morph of two contractive spectral feedback kernels
...
Внутри единого режима постоянно прогреты два contractive kernel. На нижнем конце
...
trajectory span `1.0`, body/presence depths `0.52/0.95`. Это прежнее
...
не расширяет энергию. Независимые L/R guards сохраняют contractivity, а sub не
проходит отдельный изменяемый фильтр.

При Freeze Drift продолжает спектральное движение, но возвращает только норму
feedback-вектора, потерянную contractive kernels. Это не отменяет loop gain
`0.9985` и не добавляет энергию: хвост остаётся устойчивым, но не исчезает из-за
накопленного спектрального затухания. Поведение проверяется 120-секундным
Freeze render.
...
`Character` находится первой строкой Generic Editor. Все четыре варианта
взаимоисключающие: выбор нового режима полностью заменяет предыдущий, без
дополнительных флагов и скрытых приоритетов. В версии `0.7.0` используются новые
host-ID `algorithm` и `evolution`; состояния и automation версий `0.6.0` и ниже
намеренно не мигрируются. После обновления следует удалить старый экземпляр,
выполнить полный rescan и добавить плагин заново. Высота Generic Editor равна
`420 px`, все десять строк видны без вертикальной прокрутки.
...
- оба contractive kernel единого Drift, их convex morph и sub bypass;
...
100% увеличил non-sub spectral motion с `0.0465` до `0.1049`, а stereo spectral
motion — с `0.0534` до `0.1212`; symmetric NRMS равен `0.359`. Slow sub motion
остаётся малым (`0.0355→0.0938 dB`), sub late/early ratio равен `1.018`, поэтому
...
Контрольные Release renders версии `0.7.0` используют Evolution `35%`:
...
Drift    SHA-256 c71734af2ad2f2a092cebc66d64f672d6de9de55f8ee76845ea04f1012cccd64
