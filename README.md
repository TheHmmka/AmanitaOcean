...
Ambient и Downtempo. Текущая версия `0.12.0` — проверяемый DSP-прототип с
...
- VST3, Audio Unit и CLAP targets для macOS.
...
их можно скопировать вручную после сборки. CLAP для Bitwig устанавливается в
`~/Library/Audio/Plug-Ins/CLAP/Amanita Ocean.clap`. Локальные bundle получают
финальную ad-hoc подпись после сборки; для распространения её нужно заменить
Developer ID подписью в отдельном packaging/notarization процессе.
