...
Ambient и Downtempo. Текущая версия `0.8.2` — проверяемый DSP-прототип с
...
`0.7.0` были введены host-ID `algorithm` и `evolution`; версия `0.8.2` сохраняет
...
Контрольные Release DSP renders используют Evolution `35%`. В версии `0.8.2`
DSP не изменялся, поэтому fingerprints совпадают с `0.7.0`:
