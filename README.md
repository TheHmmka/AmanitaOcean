...
Ambient и Downtempo. Текущая версия `0.10.0` — проверяемый DSP-прототип с
...
- one-knob Perceptual Ducking с dry/wet masking-анализом, пассивными
...
stereo input
    ...
    -> wet-only M/S width
    -> perceptual wet Ducking from dry/wet spectral conflict + transient detector
...
`Ducking` — один макро-параметр `0–100%`. Начиная с `0.10.0`, отдельный
`SpatialDucker` не понижает весь wet-канал вслед за громкостью dry. Он параллельно
...
| Ducking | 0–100 % (default 0 %) | Perceptual dry/wet unmasking: локальные spectral pockets, transient и stereo separation |
...
`0.9.0` добавил host-ID `ducking` с нейтральным default `0%`; `0.10.0` заменяет
его исходный full-band gain ducking на Perceptual Ducking без изменения
host-параметра. Проект пока не релизный, поэтому миграция остальных pre-release
state/automation намеренно не гарантируется. После обновления UI хосту может
