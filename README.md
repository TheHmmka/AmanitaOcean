...
Ambient и Downtempo. Текущая версия `0.9.0` — проверяемый DSP-прототип с
...
- one-knob Spatial Ducking с независимыми L/R detector и адаптивной связью
  центральных источников;
...
stereo input
    ...
    -> wet-only M/S width
    -> adaptive-stereo wet Ducking from the dry-input detector
...
`Ducking` — один макро-параметр `0–100%`. Отдельный `SpatialDucker` постоянно
анализирует уже sanitised dry-вход, но gain применяет только к готовому
wet-сигналу после Width и до Mix. Feedback, RT60 и содержимое Freeze при этом не
изменяются: хвост продолжает развиваться за gain-огибающей и полностью
возвращается после входного сигнала.

Для L/R используются независимые energy detector. 20-ms RMS-like состояние
сочетается с быстрым peak hold, а soft knee начинается около `−41 dBFS`, имеет
центр `−36 dBFS` и достигает максимальной редукции `15 dB`. Gain reduction
набирается за `10 ms`; release программно-зависимый — примерно `180–420 ms`:
глубокое приглушение возвращается мягче, мелкое освобождается быстрее. Amount
применяется после постоянно прогретой detector-огибающей и сглаживается за
`50 ms`, поэтому `0%` становится точным unity bypass без дополнительного
долгого release.

Связь каналов определяется 40-ms анализом balance и positive correlation.
Центральный коррелированный источник получает одинаковый L/R gain; чистый
hard-left или hard-right источник не приглушает противоположный канал.
Uncorrelated stereo получает только небольшую промежуточную связь против
дрожания панорамы. При переходе от центра к hard-pan старая редукция тихой
стороны отпускается отдельной быстрой пространственной огибающей, без gain-step.
Переключателей Link, Attack или Release в UI нет — вся схема остаётся одной
музыкальной ручкой.
...
| Ducking | 0–100 % (default 0 %) | Adaptive-stereo приглушение wet-хвоста входным сигналом, до 15 dB |
...
`0.9.0` добавляет host-ID `ducking` с нейтральным default `0%`. Проект пока не
релизный, поэтому миграция pre-release state/automation намеренно не
гарантируется. После обновления UI хосту может потребоваться полный rescan или
перезапуск, чтобы сбросить кэш editor bundle.
...
- Spatial Ducking при centered/hard-left/hard-right входе на всех sample rate;
- точную независимость противоположного канала, Freeze isolation и полное
  восстановление внутреннего FDN state после Ducking;
